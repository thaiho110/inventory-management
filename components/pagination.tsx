        ...
        <nav className="flex items-center justify-center gap-1">
...

            {
                visiblePages.map((page,key) => {
                    ...
                    return (
                        <Link key={key} href={getPageUrl(pageNumber)} className={`px-3 py-2 text-sm font-medium rounded-lg ${isCurrentPage ? "bg-purple-500 text-white": "text-gray-700 hover:bg-gray-100 border border-gray-300"}`}>
                        {pageNumber}
                        </Link>
                    )
